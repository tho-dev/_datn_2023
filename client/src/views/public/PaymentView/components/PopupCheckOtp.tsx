...
import { Button, FormControl, FormErrorMessage, PinInput, PinInputField, Text, Flex, useToast } from "@chakra-ui/react";
import { useCheckOtpMutation, useCreateMutation, usePaymentMomoMutation, useSendOtpMutation } from "~/redux/api/order";
...
const PopupCheckOtp = ({ isOpenOtp, onCloseOtp, dataOrder }: Props) => {
	...
	const { time, isCheckOtp } = useAppSelector((state) => state.persistedReducer.global);
			...
			<form onSubmit={submitForm}>
				...
				>
					...
					>
						...
						<Flex gap={4}>
							<Button
								bg={`${value.length < 6 ? "bg.bgEdit" : "bg.bgSuccess"}`}
								color={`${value.length < 6 ? "text.textEdit" : "text.textSuccess"}`}
								mt={"16px"}
								type="submit"
								isLoading={loading}
							>
