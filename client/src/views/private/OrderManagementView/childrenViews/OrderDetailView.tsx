...
import { Button, Image, StepIcon, Table, useDisclosure } from "@chakra-ui/react";
...
import { checkOrderStatus, checkStatusOrder, chuyenDoiSoDienThoaiVe0, formatNumber } from "~/utils/fc";
...
const OrderDetailView = (props: Props) => {
	...
	const { isOpen: isOpenOrder, onClose: onCloseOrder, onOpen: onOpenOrder } = useDisclosure();
	const { isOpen: isPDFOpen, onClose: onPDFClose, onOpen: onPDFOpen } = useDisclosure();
	...
	const [tokenPrintOrder, { isLoading: isLoadingPrint }] = useTokenPrintOrderMutation();
		...
		columnHelper.accessor("#", {
			cell: (info) => (
				<Text
					...
					fontWeight="semibold"
				>
					{(info.row.original.price * info.row.original.quantity)?.toLocaleString()}đ
				</Text>
			),
			header: "Thành tiền",
		}),
	...
	const handlePrinOrder = async () => {
		if (data?.data.status == ("processing" as any) || data?.data.status == ("confirmed" as any)) {
			...
			>
				...
				>
					...
					<Box fontWeight="semibold">
						...
						<Text
							...
							fontSize="13px"
						>
							{data?.data.payment_method.message == "failed" ? "Chưa thanh toán" : "Đã thanh toán"}
						</Text>
					</Box>
				</OrderDetailMetricItem>
				<OrderDetailMetricItem
				...
				>
					...
					>
						...
						{data.data.products.reduce((acc: any, product: any) => acc + product.quantity, 0)}
					</Text>
					<Text
						fontSize="13px"
						fontWeight="semibold"
					...
					>
						...
						{formatNumber(`${data?.data.shipping_info?.transportation_fee || 0}`)} VND
					</Text>
					...
					>
						...
						{formatNumber(
							`${data?.data.total_amount + (data?.data.shipping_info?.transportation_fee || 0)}`
						)}{" "}
						VND
					</Text>
				</OrderDetailMetricItem>
