...
import { ExcelIcon, FilterIcon, SearchIcon } from "~/components/common/Icons";
import { useGetAllOrderQuery } from "~/redux/api/order";
import SelectThinkPro from "~/components/SelectThinkPro";
import { checkOrderStatus, chuyenDoiSoDienThoaiVe0, objectToUrlParams } from "~/utils/fc";
...
const OrderManagementView = () => {
				...
				return (
					<Menu>
						...
						<MenuList>
							...
							<MenuItem>
								<Link to={`/admin/don-hang/cap-nhat/${info.getValue()}`}>Cập nhật</Link>
							</MenuItem>
						</MenuList>
					</Menu>
				);
	...
	const handleDownloadExcel = async () => {
		fetch(process.env.VITE_API_URL + `/order/export-excel?${objectToUrlParams(debouncedSearchTerm)}`)
			.then((response) => response.blob())
			.then((blob) => {
				const url = window.URL.createObjectURL(blob);
				const a = document.createElement("a");
				a.href = url;
				a.download = "file_orders.xlsx";
