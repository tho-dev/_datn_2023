...
import { useDeleteOptionProductMutation, useSaveVariantsMutation } from "~/redux/api/product";
...
const OptionNested = ({ nestIndex, control, register, errors, watch }: any) => {
	...
	const lengthOptions = watch(`variants.${[nestIndex]}.options`)?.length;
...
const Options = ({ control, register, errors, setValue, getValues, watch }: Props) => {
	...
	const handleDeleteOption = async (isDisabled: boolean, lengthOptions: number, index: number) => {
								...
								>
									{fields.map((item, index) => {
										return (
											...
											>
												{(provided, snapshot) => {
													const isDisabled = watch(`variants.${[index]}.option_id`)
														? true
														: false;

													const lengthOptions = watch(`variants.${[index]}.options`)?.length;
